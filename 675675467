...
-- ==========================================
-- [ MAPA SIEMPRE SOLIDO - SOLO NATURAL DISASTER ]
-- ==========================================
local IS_NDS = (game.PlaceId == 189707)
if IS_NDS then
    local mapKwNDS = {
        "floor","wall","roof","base","ground","platform","stair","step",
        "union","tile","plank","brick","road","building","struct","solid",
        "ramp","island","terrain","land","bridge","rail","fence","pillar",
        "column","ceil","ceiling","edge","border","spawn","water"
    }
    local function isPlayerCharacterPartNDS(part)
    ...
    local function solidifyPart(part)
        ...
        if isPlayerCharacterPartNDS(part) then return end
        part.CanCollide = true
        if not part.Anchored then return end
        local n = string.lower(part.Name)
        for _, kw in ipairs(mapKwNDS) do
            if string.find(n, kw, 1, true) then part.Anchored = true return end
        end
    end
    Workspace.DescendantAdded:Connect(function(obj)
        task.defer(function() pcall(solidifyPart, obj) end)
    end)
    task.spawn(function()
        local all = Workspace:GetDescendants()
        local batch = 50
        for i = 1, #all, batch do
            for j = i, math.min(i + batch - 1, #all) do pcall(solidifyPart, all[j]) end
            task.wait()
        end
    end)
end
...
local function addPart(part)
    if RetainPart(part) then
        if not table.find(partsRing, part) then table.insert(partsRing, part) end
    end
end
local function removePart(part)
    local index = table.find(partsRing, part)
    if index then table.remove(partsRing, index) end
end
    ...
    connections.hunt = RunService.Heartbeat:Connect(function(dt)
        ...
        pcall(function()
            ...
            if tHrp and tHum and tHum.Health > 0 then
                ...
                hrp.AssemblyLinearVelocity = Vector3.zero
                pcall(function() tHrp.AssemblyAngularVelocity = Vector3.new(999999999, 999999999, 999999999) end)
            else
                stopHunt()
                pcall(function()
                    hrp.AssemblyLinearVelocity = Vector3.zero
        ...
        if ok and result then
            ...
            if jok and data and data.items then
                for _, item in ipairs(data.items) do
                    -- type=2 en bundleItems = animation asset
                    if item.type == "UserOutfit" or (item.id and item.type ~= "Asset") then
                        -- nada
                    elseif item.id then
                        callback(item.id)
                        return
